...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    const lastTime =
      !isMobile && layerStates.length ? (layerStates.length - 1) * 0.07 : 0;
    const panelInsertTime =
      lastTime + (!isMobile && layerStates.length ? 0.08 : 0);
  ...
  const playClose = useCallback(() => {
    ...
    const isMobile = window.innerWidth < 640;
    const all: HTMLElement[] = isMobile ? [panel] : [...layers, panel];
    closeTweenRef.current?.kill();
  ...
  return (
    <div
      className={`sm-scope z-[100] ${isFixed ? "pointer-events-none fixed top-0 left-0 h-screen w-screen overflow-hidden" : "h-full w-full"}`}
      data-open={open || undefined}
    >
      <div
        className={
          (className ? className + " " : "") +
          "staggered-menu-wrapper pointer-events-none relative z-[100] h-full w-full"
        }
        style={
          {
      ...
      >
        <div
          ref={preLayersRef}
          className="sm-prelayers pointer-events-none absolute top-0 right-0 bottom-0 z-[105] hidden sm:block"
          aria-hidden="true"
        >
          {(() => {
            const raw =
        ...
        >
          ...
          >
            <Link href="/" className="block">
              <Image
                src={open ? logoDarkUrl : logoUrl}
                alt="Logo"
                className="sm-logo-img block h-8 w-auto object-contain transition-opacity duration-300"
                width={110}
                height={32}
                priority
                unoptimized
...
@media (min-width: 640px) {
  .sm-scope .staggered-menu-panel { width: clamp(260px, 38vw, 420px); padding: 6em 2em 2em 2em; }
